...
module.exports = {
  ...
  handlePrefixCommand: async function (message, cmd, settings) {
    ...
    if (cmd.category === "OWNER" && !OWNER_IDS.includes(message.author.id)) {
      return message.safeReply("Este comando solo es accesible para los propietarios de arctic.");
    }
    ...
    if (cmd.userPermissions && cmd.userPermissions?.length > 0) {
      if (!message.channel.permissionsFor(message.member).has(cmd.userPermissions)) {
        return message.safeReply(`Necesitas ${parsePermissions(cmd.userPermissions)} para este comando`);
      }
    }
    ...
    if (cmd.botPermissions && cmd.botPermissions.length > 0) {
      if (!message.channel.permissionsFor(message.guild.members.me).has(cmd.botPermissions)) {
        return message.safeReply(`Necesito ${parsePermissions(cmd.botPermissions)} para este comando`);
      }
    }
    ...
    if (cmd.cooldown > 0) {
      ...
      if (remaining > 0) {
        return message.safeReply(`Estás en tiempo de reutilización. Puedes volver a utilizar el comando en \`${timeformat(remaining)}\``);
      }
    }
    ...
    try {
      ...
      message.safeReply("Se produjo un error al ejecutar este comando.");
    } finally {
  ...
  handleSlashCommand: async function (interaction) {
    ...
    if (!cmd) return interaction.reply({ content: "Se ha producido un error", ephemeral: true }).catch(() => { });
    ...
    if (cmd.category === "OWNER" && !OWNER_IDS.includes(interaction.user.id)) {
      return interaction.reply({
        content: `Este comando solo es accesible para los propietarios de bots.`,
        ephemeral: true,
      });
    }
    ...
    if (interaction.member && cmd.userPermissions?.length > 0) {
      if (!interaction.member.permissions.has(cmd.userPermissions)) {
        return interaction.reply({
          content: `Necesitas ${parsePermissions(cmd.userPermissions)} para este comando`,
          ephemeral: true,
        });
      }
    }
    ...
    if (cmd.botPermissions && cmd.botPermissions.length > 0) {
      if (!interaction.guild.members.me.permissions.has(cmd.botPermissions)) {
        return interaction.reply({
          content: `Necesito ${parsePermissions(cmd.botPermissions)} para este comando`,
          ephemeral: true,
        });
      }
    }
    ...
    if (cmd.cooldown > 0) {
      ...
      if (remaining > 0) {
        return interaction.reply({
          content: `Estás en tiempo de reutilización. Puedes volver a utilizar el comando en \`${timeformat(remaining)}\``,
          ephemeral: true,
        });
      }
    }
    ...
    try {
      ...
      await interaction.followUp("Oops! Se produjo un error al ejecutar el comando.");

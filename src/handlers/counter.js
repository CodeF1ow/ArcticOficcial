  ...
  client.counterUpdateQueue.forEach(async (guildId) => {
    ...
    try {
      ...
      for (const config of settings.counters) {
        ...
        if (vc.manageable) vc.setName(channelName).catch((err) => vc.client.logger.log("Error al configurar el nombre: ", err));
      }
    } catch (ex) {
      client.logger.error(`Error al actualizar los canales de contador para guildId: ${guildId}`, ex);
    } finally {

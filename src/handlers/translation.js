...
async function handleFlagReaction(emoji, message, user) {
  ...
  if (remaining > 0) {
    return message.channel.safeSend(`${user} Debes esperar ${timeformat(remaining)} antes de traducir de nuevo!`, 5);
  }
  ...
  // filtrar los idiomas disponibles para la traducción en Google Translate
  ...
  const embed = new EmbedBuilder()
    ...
    .setAuthor({ name: `Traducción de ${src}` })
    .setDescription(desc)
    .setFooter({
  ...
  message.channel.safeSend({ embeds: [embed], components: [btnRow] }).then(
    () => cooldownCache.set(user.id, Date.now()) // establecer tiempo de espera
  );

...
async function performAutomod(message, settings) {
  ...
  if (mentions.members.size > automod.max_mentions) {
    fields.push({ name: "Menciones", value: `${mentions.members.size}/${automod.max_mentions}`, inline: true });
  ...
  if (automod.anti_massmention > 0) {
    ...
    if (mentions.everyone) {
      fields.push({ name: "Todos mencionan", value: "✓", inline: true });
      strikesTotal += 1;
    }
  ...
  if (automod.anti_attachments) {
    if (message.attachments.size > 0) {
      fields.push({ name: "Adjuntos encontrados", value: "✓", inline: true });
      shouldDelete = true;
      strikesTotal += 1;
    }
  }
  ...
  if (shouldDelete && message.deletable) {
    message
      .delete()
      .then(() => channel.safeSend("> Moderación automática! Mensaje borrado", 5))
      .catch(() => { });
  }
  ...
  if (strikesTotal > 0) {
    ...
    if (logChannel) {
      const logEmbed = new EmbedBuilder()
        ...
        .setFooter({
          text: `Por ${author.username} | ${author.id}`,
    ...
    const strikeEmbed = new EmbedBuilder()
      ...
      .addFields(fields)
      .setDescription(
        `Has recibido ${strikesTotal} avisos!\n\n` +
        `**Servidor:** ${guild.name}\n` +
        `**Total Strikes:** ${memberDb.strikes} fuera de ${automod.strikes}`
      );

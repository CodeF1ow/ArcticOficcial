...
const logModeration = async (issuer, target, reason, type, data = {}) => {
  ...
  if (type.toUpperCase() !== "PURGE") {
    ...
    fields.push({ name: "Razón", value: reason || "No se proporcionó ninguna razón", inline: false });
    ...
    if (type.toUpperCase() === "MOVE") {
      fields.push({ name: "Trasladado a", value: data.channel.name, inline: true });
    }
  }
...
module.exports = class ModUtils {
  ...
  static async warnTarget(issuer, target, reason) {
    ...
    try {
      ...
      if (memberDb.warnings >= settings.max_warn.limit) {
        await ModUtils.addModAction(issuer.guild.members.me, target, "Se alcanzó el máximo de advertencias", settings.max_warn.action); // moderate
  ...
  static async deafenTarget(issuer, target, reason) {
    ...
    try {
      ...
      return `No pudo ensordecer ${target.user.tag}`;
    }
  }
  ...
  static async moveTarget(issuer, target, reason, channel) {
    ...
    try {
      ...
      logModeration(issuer, target, reason, "Mover", { channel });
      return true;
    } catch (ex) {
      error(`moveTarget`, ex);
      return "ERROR";

  ...
  async messageRun(message, args, data) {
    ...
    switch (type) {
      ...
      case "rep":
        ...
        break;
      default:
        response = "Tipo de Leaderboard no válido. Seleccione `xp`, `invite`o `rep`";
    }
...
async function getXpLeaderboard({ guild }, author, settings) {
  ...
  if (!settings.stats.enabled) return "La tabla de clasificación está desactivada en este servidor";
...
async function getInviteLeaderboard({ guild }, author, settings) {
  ...
  const embed = new EmbedBuilder()
    .setAuthor({ name: "Leaderboard Invitaciones" })
    .setColor(EMBED_COLORS.BOT_EMBED)
    .setDescription(collector)
    .setFooter({ text: `Solicitado por${author.tag}` });
...
async function getRepLeaderboard(author) {
  ...
  const embed = new EmbedBuilder()
    .setAuthor({ name: "Leaderboard Reputacion" })
    .setColor(EMBED_COLORS.BOT_EMBED)
    .setDescription(collector)
    .setFooter({ text: `Solicitado por ${author.tag}` });

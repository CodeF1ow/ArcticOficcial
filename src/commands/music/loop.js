...
module.exports = {
  name: "loop",
  description: "repite la canción o la cola",
  category: "MUSIC",
    ...
    options: [
      {
        ...
        description: "La entidad que desea hacer un bucle",
        required: false,
        choices: [
          {
            name: "queue",
...
function toggleLoop({ client, guildId }, type) {
  ...
  if (type === "track") {
    ...
    return "El modo de bucle está configurado en `track`";
  }

  // cola
  else if (type === "queue") {
    player.queue.setLoop(1);
    return "El modo de bucle está configurado en `queue`";
  }
}

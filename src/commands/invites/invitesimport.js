...
module.exports = {
  name: "invitesimport",
  description: "Agrega invitaciones de servidores existentes a los usuarios",
  category: "INVITE",
  ...
  slashCommand: {
    ...
    options: [
      {
        name: "user",
        description: "usuario para importar invitaciones para",
...
async function importInvites({ guild }, user) {
  if (user && user.bot) return "Oops! No puedes importar invitaciones para bots";
  ...
  return `Hecho! Invitaciones anteriores añadidas a ${user ? user.username : "todos los miembros"}`;
}

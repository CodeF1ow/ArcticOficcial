...
module.exports = {
  ...
  slashCommand: {
    ...
    options: [
      {
        name: "category",
        description: "categoría del meme",
...
async function getRandomEmbed(choice) {
  ...
  if (!response.success) {
    return new EmbedBuilder().setColor(EMBED_COLORS.ERROR).setDescription("Error al obtener meme.");
  }

  const json = response.data;
  if (!Array.isArray(json) || json.length === 0) {
    return new EmbedBuilder().setColor(EMBED_COLORS.ERROR).setDescription(`Ningún meme encontrado para la categoría ${choice}`);
  }

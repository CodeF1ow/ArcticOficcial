...
async function untimeout(issuer, target, reason) {
  ...
  if (typeof response === "boolean") return `Se elimina el tiempo de espera de ${target.user.username}!`;
  ...
  else if (response === "NO_TIMEOUT") return `${target.user.username} no está agotado!`;

...
async function getMatchingBans(guild, match) {
  ...
  if (matched.length === 0) return `No se encontró ningún usuario que coincida ${match}`;
  ...
  return { content: "Por favor seleccione un usuario que desee desbanear", components: [menuRow] };
}
...
async function waitForBan(issuer, reason, sent) {
  ...
  collector.on("collect", async (response) => {
    ...
    if (typeof status === "boolean") return sent.edit({ content: `¡${user.username} no está baneado!`, components: [] });
    else return sent.edit({ content: `No se pudo desbanear ${user.username}`, components: [] });
  });

  // collect user and unban
  collector.on("end", async (collected) => {
    if (collected.size === 0) return sent.edit("Oops! Timed out. Vuelve a intentarlo más tarde.");
  });
}

...
module.exports = {
  name: "kick",
  description: "patea el miembro especificado",
  category: "MODERATION",
  ...
  slashCommand: {
    ...
    options: [
      ...
      {
        name: "reason",
        description: "motivo de la patada",
...
async function kick(issuer, target, reason) {
  ...
  if (typeof response === "boolean") return `${target.user.username} es expulsado!`;
  if (response === "BOT_PERM") return `no tengo permiso para patear ${target.user.username}`;
  else if (response === "MEMBER_PERM") return `No tienes permiso para patear ${target.user.username}`;
  else return `No se pudo patear ${target.user.username}`;
}

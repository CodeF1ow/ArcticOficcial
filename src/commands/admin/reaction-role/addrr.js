...
module.exports = {
  name: "addrr",
  description: "configura el mensaje de rol de reacción en el mensaje especificado",
  category: "ADMIN",
  ...
  slashCommand: {
    ...
    options: [
      ...
      {
        name: "message_id",
        description: "ID del mensaje al que se le deben configurar los roles de reacción",
      ...
      {
        name: "rol",
        description: "rol que se otorgará al emoji seleccionado",
...
async function addRR(guild, channel, messageId, reaction, role) {
  ...
  if (role.managed) {
    return "No puedo asignar roles de bot";
  }

  if (guild.roles.everyone.id === role.id) {
    return "No puedes asignar el rol everyone";
  }

  if (guild.members.me.roles.highest.position < role.position) {
    return "Ups! No puedo añadir/quitar miembros a ese rol. ¿Ese rol es superior al mío?";
  }
  ...
  try {
    ...
    return `Ups! Fallo al reaccionar. ¿Es este un emoji válido: ${reaction} ?`;
  }
  ...
  return (reply += "Listo! La configuración se ha guardado.");
}
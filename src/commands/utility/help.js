...
module.exports = {
  name: "help",
  description: "menú de ayuda del comando",
  category: "UTILITY",
  ...
  async messageRun(message, args, data) {
    ...
    await message.safeReply("No se encontró ningún comando coincidente");
  },
  ...
  async interactionRun(interaction) {
    ...
    // No matching command/category found
    await interaction.followUp("No se encontró ningún comando coincidente");
  },
};
...
async function getHelpMenu({ client, guild }) {
  ...
  for (const [k, v] of Object.entries(CommandCategory)) {
    ...
    options.push({
      label: v.name,
      value: k,
      description: `Ver comandos en categoría ${v.name}`,
      emoji: v.emoji,
    });
  }
  ...
  const menuRow = new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder()
      ...
      .setPlaceholder("Elija la categoría de comando")
      .addOptions(options)
  );
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      ...
      "Un genial bot de discord multipropósito que puede satisfacer todas tus necesidades\n\n" +
...
function getMsgCategoryEmbeds(client, category, prefix) {
  ...
  if (category === "IMAGE") {
    ...
    collector +=
      ...
      `**${prefix}cmd:** Elige el avatar de los autores del mensaje como imagen\n` +
      `**${prefix}cmd <@member>:** Elige el avatar de los miembros mencionados como imagen.\n` +
      `**${prefix}cmd <url>:** Elige la imagen proporcionada URL\n` +
      `**${prefix}cmd [attachment]:** Imagen adjunta de selecciones`;
  ...
  arrSplitted.forEach((item, index) => {
    const embed = new EmbedBuilder()
      ...
      .setDescription(item.join("\n"))
      .setFooter({
        text: `página ${index + 1} de ${arrSplitted.length} | Tipo ${prefix}help <command> para más información de comando`,
      });
    arrEmbeds.push(embed);
  });
